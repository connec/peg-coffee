(function() {
  ...
  module.exports = PegCoffeeParser = (function(_super) {
    ...
    PegCoffeeParser.prototype.RuleContent = function() {
      var action;
      action = function(_arg) {
        var action, extract, head, tail;
        head = _arg.head, tail = _arg.tail, action = _arg.action;
        if ((action != null) && tail.length > 0) {
          extract = this.extract;
          return function() {
            return this.action(this.any, [head].concat(extract(tail, 2)), action);
          };
          ...
          return function() {
            return this.action(head, action);
          };
        } else {
          return head;
        }
      };
      return this.action(this.all, [[this.label, 'head', this.RuleLine], [this.label, 'tail', this.maybe_some, this.all, [this.NEWLINE, [this.literal, '/'], this.SPACE, this.RuleLine]], [this.maybe, this.all, [this.INDENT, [this.label, 'action', this.Action]]]], action);
    };
    ...
    PegCoffeeParser.prototype.Action = function() {
      ...
      action_block = function(_arg) {
        var $$, $code;
        $$ = _arg.$$;
        $code = this.join($$.slice(2)).trim().replace(/^    /gm, '');
      ...
      action_inline = function(_arg) {
        var $$, $code;
        $$ = _arg.$$;
        $code = this.join($$.slice(1)).trim().replace(/^    /gm, '');
        return function(context) {
          var k, v;
          for (k in context) {
            if (!__hasProp.call(context, k)) continue;
